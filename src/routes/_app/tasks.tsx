...
function TasksPage() {
  ...
  useEffect(() => { load(); }, [user]);
  useEffect(() => {
    const ch = supabase.channel("tasks-list-rt")
            ...
            {filtered.map((t) => (
              <li key={t.id}>
                <button onClick={() => setActive(t)}
                  ...
                  <Badge className={`shrink-0 ${badge(t.status)}`}>{t.status.replace("_"," ")}</Badge>
                </button>
              </li>
            ))}
          </ul>
...
const dotColor = (s: string) => s === "completed" ? "bg-success" : s === "in_progress" ? "bg-gold-shine"
  : s === "blocked" ? "bg-destructive" : "bg-muted-foreground";
const badge = (s: string) => s === "completed" ? "bg-success/20 text-success border border-success/40"
  : s === "in_progress" ? "bg-primary/20 text-primary border border-primary/40"
  : s === "blocked" ? "bg-destructive/20 text-destructive border border-destructive/40"
