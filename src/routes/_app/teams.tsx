...
import { Plus, Users, Trash2, MessageSquare, Search, Crown, Loader2, Pencil, Check, X } from "lucide-react";
...
interface Member {
  id: string; team_id: string; user_id: string; joined_at: string;
}
interface Profile {
...
function TeamsPage() {
  ...
  const load = async () => {
    ...
    if (ids.length) {
      ...
      const userIds = Array.from(new Set([...(m ?? []).map((x: any) => x.user_id), ...(t ?? []).map((x: any) => x.owner_id)]));
      if (userIds.length) {
  ...
  useEffect(() => { load(); }, [user]);
  useEffect(() => {
    const ch = supabase.channel("teams-rt")
  ...
  const teamMembers = useMemo(() => members.filter((m) => m.team_id === selectedId), [members, selectedId]);
  ...
  const deleteTeam = async () => {
    if (!selected) return;
    if (!confirm(`Delete team "${selected.name}"? This removes all members and chat history.`)) return;
  ...
  if (teams === null) {
    return <div className="space-y-3">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-14" />)}</div>;
  }
    ...
    <div className="space-y-6">
      ...
      {teams.length === 0 ? (
        <Card className="surface border-border/60 p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isAdmin ? "Create your first team to start collaborating." : "Ask an admin to add you to a team."}
          </p>
        </Card>
      ) : (
          ...
          <Card className="surface border-border/60 overflow-hidden">
            <ul className="divide-y divide-border/40">
              {teams.map((t) => {
                ...
                return (
                  <li key={t.id}>
                    <button onClick={() => { setSelectedId(t.id); setEditing(false); }}
                      className={`w-full text-left px-4 py-3 transition flex items-center gap-3 ${
                        active ? "bg-gold-shine/10 border-l-2 border-gold-shine" : "hover:bg-accent/30"
                      }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1">
                      ...
                      Owner: {profiles[selected.owner_id]?.display_name ?? profiles[selected.owner_id]?.email ?? "—"}
                    </span>
                    ...
                    {teamMembers.map((m) => {
                      ...
                      return (
                        <li key={m.id}
                          ...
                          {isOwner && <Badge className="bg-gold-shine/20 text-gold-shine border-gold-shine/40">Owner</Badge>}
                          {isAdmin && !isOwner && (
                            <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive"
      ...
      {selected && (
        <AddMemberDialog open={addMemberOpen} onOpenChange={setAddMemberOpen}
          team={selected} existingMemberIds={teamMembers.map((m) => m.user_id)} onAdded={load} />
      )}
    </div>
  );
}
...
function CreateTeamDialog({ open, onOpenChange, onCreated }: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle className="font-display">New team</DialogTitle></DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          <div>
            <Label htmlFor="tn">Name</Label>
            <Input id="tn" required minLength={2} maxLength={80} value={name}
              onChange={(e) => setName(e.target.value)} className="mt-1.5" />
          </div>
          <div>
            <Label htmlFor="td">Description</Label>
            <Textarea id="td" maxLength={300} rows={3} value={description}
              onChange={(e) => setDescription(e.target.value)} className="mt-1.5" />
          </div>
          <DialogFooter>
...
function AddMemberDialog({ open, onOpenChange, team, existingMemberIds, onAdded }: {
  open: boolean; onOpenChange: (o: boolean) => void;
  team: Team; existingMemberIds: string[]; onAdded: () => void;
}) {
  ...
  const add = async (p: Profile) => {
    setBusy(true);
    const { error } = await supabase.from("team_members").insert({ team_id: team.id, user_id: p.id });
    setBusy(false);
    if (error) return toast.error(error.message);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle className="font-display">Add member to {team.name}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="relative">
            <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input autoFocus placeholder="Search by email or name…" value={q}
            ...
            {results.length === 0 ? (
              ...
              return (
                <div key={p.id} className="flex items-center gap-3 p-3">
                  ...
                  <Button size="sm" disabled={already || busy} onClick={() => add(p)}
